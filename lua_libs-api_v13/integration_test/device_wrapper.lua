...
local METRIC_NAME = "Device"
local base_protect = base_device.Device._protect

local function wrapped_init(cls, driver, raw_device)
  local device_init = base_init
  if _G.__instrument_interest[METRIC_NAME] then
    local instrumented_init, metrics = memtracer.instrument_and_register(base_init, METRIC_NAME)
    _G.memtracer_metrics[METRIC_NAME] = metrics
    device_init = instrumented_init
  end
  local dev = device_init(cls, driver, raw_device)
...
function MockDevice:expect_native_cmd_handler_registration(capability_id, capability_cmd_id)
  local devices_api = rawget(self, "devices_api")
  devices_api.__expect_native_cmd_handler_registration(self.id, capability_id, capability_cmd_id)
end

...
local function wrapped_driver_init(...)
  local driver_init = original_driver_init
  if _G.__instrument_interest[METRIC_NAME] then
    local instrumented_init, metrics = memtracer.instrument_and_register(original_driver_init, METRIC_NAME)
    _G.memtracer_metrics[METRIC_NAME] = metrics
    driver_init = instrumented_init
  end
  wrapped_driver.driver_under_test = driver_init(...)

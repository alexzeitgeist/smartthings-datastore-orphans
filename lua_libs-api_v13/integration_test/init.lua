...
if _capability_awareness == nil then
  _capability_awareness = true
end
...
socket:add_mock_socket("devices", mock_devices_api)
...
function integration_test.instrument_device_init()
  _G.__instrument_interest["Device"] = true
end
function integration_test.instrument_driver_init()
  _G.__instrument_interest["Driver"] = true
end
...
function integration_test.register_test(test_name, test_func, test_init, expected_error)
  local test_name = string.gsub(test_name, "\n", " ")
...
function maybe_print_metrics()
  if not next(_G.__instrument_interest) then
    return
  end
  local memtracer = require "st.memtracer"
  assert(io.stderr:write("=====METRICS REPORTS=====\n"))
  for k, v in pairs(_G.__instrument_interest) do
    assert(io.stderr:write(k .. "\n"))
    local _, metric = table.unpack(memtracer.instrumented_function.registered_fns[k])
    local rep = assert(metric:generate_report())
    assert(io.stderr:write(string.format("min: %.2f\n", rep.min)))
    assert(io.stderr:write(string.format("max: %.2f\n", rep.max)))
    assert(io.stderr:write(string.format("avg: %.2f\n", rep.mean)))
    assert(io.stderr:write(string.format("med: %.2f\n", rep.median)))
    assert(io.stderr:write(string.format("std: %.2f\n", rep.std_dev)))
    assert(io.stderr:write(string.format("cnt: %.2f\n", rep.count)))
  end
end

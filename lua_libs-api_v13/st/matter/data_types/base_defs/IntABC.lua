...
function IntABC.new_mt(base, byte_length)
  ...
  mt.__call = function(orig, val)
    ...
    elseif val > (1 << ((byte_length * 8) - 1)) - 1 then
      error(string.format("%s value too large for type", orig.NAME), 2)
    elseif val < -1 << ((byte_length * 8) - 1) then
      error(string.format("%s value too negative for type", orig.NAME), 2)
    end
    local o = {}
    setmetatable(o, mt)
    o.value = val

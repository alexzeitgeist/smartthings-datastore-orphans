...
function FloatABC.new_mt(base, byte_length, mantissa_bit_length, exponent_bit_length)
  ...
  i_table.serialize = function(self, buf, include_control_octet, tag)
    ...
    local full_bits = {}
    table.insert(full_bits, self.sign_bit)
    local exp_bytes = math.ceil(self.exponent_bit_length / 8)
    local exp_bits = utils.bitify(utils.serialize_int(self.exponent + self.exponent_modifier, exp_bytes, false, false))
    for i = ((exp_bytes * 8) - self.exponent_bit_length) + 1, (exp_bytes * 8) do
      table.insert(full_bits, exp_bits[i])
    end

    local man_bits = mantissa_to_bits(self.mantissa, self.mantissa_bit_length)
    for _, bit in ipairs(man_bits) do
      table.insert(full_bits, bit)
    end
    local float_bytes = string.reverse(utils.bit_list_to_byte_str(full_bits))
    buf:write_bytes(float_bytes)
    return buf.buf
  end

...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.Switch.server.attributes")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      Switch.attribute_direction_map[alias] = "server"
    end
  end
end
...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.Switch.server.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      Switch.command_direction_map[alias] = "server"
    end
  end
end

do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.Switch.client.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      Switch.command_direction_map[alias] = "client"
    end
  end
end

Switch.FeatureMap = Switch.types.Feature

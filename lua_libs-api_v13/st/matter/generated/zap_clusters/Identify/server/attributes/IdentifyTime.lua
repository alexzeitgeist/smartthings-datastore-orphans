...
--- @class st.matter.clusters.Identify.IdentifyTime
--- @alias IdentifyTime
---
...
function IdentifyTime:read(device, endpoint_id)
...
--- @param device st.matter.Device
--- @param endpoint_id number|nil
--- @param value st.matter.data_types.Uint16 the value to write
--- @return st.matter.interaction_model.InteractionRequest
function IdentifyTime:write(device, endpoint_id, value)
...
--- Reporting policy: IdentifyTime => true => mandatory
...
--- @param endpoint_id number|nil
--- @return any
function IdentifyTime:subscribe(device, endpoint_id)
...
--- Builds an IdentifyTime test attribute reponse for the driver integration testing framework
---
...
function IdentifyTime:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function IdentifyTime:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(IdentifyTime, {__call = IdentifyTime.new_value, __index = IdentifyTime.base_type})
return IdentifyTime

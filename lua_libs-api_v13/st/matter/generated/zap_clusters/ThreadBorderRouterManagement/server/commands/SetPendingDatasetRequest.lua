...
-----------------------------------------------------------
-- ThreadBorderRouterManagement command SetPendingDatasetRequest
-----------------------------------------------------------

--- @class st.matter.clusters.ThreadBorderRouterManagement.SetPendingDatasetRequest
--- @alias SetPendingDatasetRequest
---
--- @field public ID number 0x0004 the ID of this command
--- @field public NAME string "SetPendingDatasetRequest" the name of this command
--- @field public pending_dataset st.matter.data_types.OctetString1
local SetPendingDatasetRequest = {}

SetPendingDatasetRequest.NAME = "SetPendingDatasetRequest"
SetPendingDatasetRequest.ID = 0x0004
SetPendingDatasetRequest.field_defs = {
  {
    name = "pending_dataset",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
...
--- Builds an SetPendingDatasetRequest test command reponse for the driver integration testing framework
---
...
function SetPendingDatasetRequest:build_test_command_response(device, endpoint_id, status)
...
--- Initialize the SetPendingDatasetRequest command
---
--- @param self SetPendingDatasetRequest the template class for this command
--- @param device st.matter.Device the device to build this message to
--- @param pending_dataset st.matter.data_types.OctetString1

--- @return st.matter.interaction_model.InteractionRequest of type INVOKE
function SetPendingDatasetRequest:init(device, endpoint_id, pending_dataset)
  local out = {}
  local args = {pending_dataset}
  ...
  setmetatable(out, {
    __index = SetPendingDatasetRequest,
    __tostring = SetPendingDatasetRequest.pretty_print
  })
...
function SetPendingDatasetRequest:set_parent_cluster(cluster)
...
function SetPendingDatasetRequest:augment_type(base_type_obj)
  local elems = {}
...
function SetPendingDatasetRequest:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(SetPendingDatasetRequest, {__call = SetPendingDatasetRequest.init})

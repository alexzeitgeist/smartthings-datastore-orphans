...
--- @class st.matter.clusters.SmokeCoAlarm.SelfTestComplete
--- @alias SelfTestComplete
---
--- @field public ID number 0x0005 the ID of this event
--- @field public NAME string "SelfTestComplete" the name of this event
local SelfTestComplete = {
  ID = 0x0005,
  NAME = "SelfTestComplete",
  base_type = data_types.Structure,
}

SelfTestComplete.field_defs = {
}
--- Add field names to each event field
---
--- @param base_type_obj st.matter.data_types.Structure
function SelfTestComplete:augment_type(base_type_obj)
  local elems = {}
...
function SelfTestComplete:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function SelfTestComplete:subscribe(device, endpoint_id)
...
function SelfTestComplete:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "SelfTestCompleteEventData", ID = 0x15}))
...
function SelfTestComplete:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function SelfTestComplete:set_parent_cluster(cluster)

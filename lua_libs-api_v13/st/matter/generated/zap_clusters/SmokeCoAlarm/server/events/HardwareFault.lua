...
--- @class st.matter.clusters.SmokeCoAlarm.HardwareFault
--- @alias HardwareFault
---
--- @field public ID number 0x0003 the ID of this event
--- @field public NAME string "HardwareFault" the name of this event
local HardwareFault = {
  ID = 0x0003,
  NAME = "HardwareFault",
...
function HardwareFault:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function HardwareFault:subscribe(device, endpoint_id)
...
function HardwareFault:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "HardwareFaultEventData", ID = 0x15}))
...
function HardwareFault:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function HardwareFault:set_parent_cluster(cluster)

...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.FanControl.server.attributes")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      FanControl.attribute_direction_map[alias] = "server"
    end
  end
end
...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.FanControl.server.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      FanControl.command_direction_map[alias] = "server"
    end
  end
end

do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.FanControl.client.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      FanControl.command_direction_map[alias] = "client"
    end
  end
end

FanControl.FeatureMap = FanControl.types.Feature

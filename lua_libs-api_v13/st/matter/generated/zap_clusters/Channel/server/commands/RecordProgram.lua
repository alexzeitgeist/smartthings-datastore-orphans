...
--- @field public NAME string "RecordProgram" the name of this command
...
RecordProgram.NAME = "RecordProgram"
RecordProgram.ID = 0x0006
RecordProgram.field_defs = {
  {
...
function RecordProgram:build_test_command_response(device, endpoint_id, status)
...
---
--- @param self RecordProgram the template class for this command
--- @param device st.matter.Device the device to build this message to
...
function RecordProgram:init(device, endpoint_id, program_identifier, should_record_series, externalid_list, data)
  local out = {}
  ...
  setmetatable(out, {
    __index = RecordProgram,
    __tostring = RecordProgram.pretty_print
  })
...
function RecordProgram:set_parent_cluster(cluster)
...
function RecordProgram:augment_type(base_type_obj)
  local elems = {}
...
function RecordProgram:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(RecordProgram, {__call = RecordProgram.init})

...
--- @class st.matter.clusters.DeviceEnergyManagement.Resumed
--- @alias Resumed
---
--- @field public ID number 0x0003 the ID of this event
--- @field public NAME string "Resumed" the name of this event
...
function Resumed:augment_type(base_type_obj)
  local elems = {}
...
function Resumed:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function Resumed:subscribe(device, endpoint_id)
...
function Resumed:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "ResumedEventData", ID = 0x15}))
...
function Resumed:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function Resumed:set_parent_cluster(cluster)

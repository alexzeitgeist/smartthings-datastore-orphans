...
--- @class st.matter.clusters.DeviceEnergyManagement.Paused
--- @alias Paused
---
--- @field public ID number 0x0002 the ID of this event
--- @field public NAME string "Paused" the name of this event
local Paused = {
  ID = 0x0002,
  NAME = "Paused",
...
function Paused:augment_type(base_type_obj)
  local elems = {}
...
function Paused:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function Paused:subscribe(device, endpoint_id)
...
function Paused:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "PausedEventData", ID = 0x15}))
...
function Paused:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function Paused:set_parent_cluster(cluster)

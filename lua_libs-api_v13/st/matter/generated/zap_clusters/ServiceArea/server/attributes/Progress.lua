...
--- @field public NAME string "Progress" the name of this attribute
...
local Progress = {
  ...
  element_type = require "st.matter.generated.zap_clusters.ServiceArea.types.ProgressStruct",
}

--- Add additional functionality to the base type object
---
--- @param base_type_obj st.matter.data_types.Array the base data type object to add functionality to
function Progress:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, Progress.element_type)
  end
end
...
function Progress:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function Progress:subscribe(device, endpoint_id)
...
function Progress:set_parent_cluster(cluster)
...
function Progress:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function Progress:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(Progress, {__call = Progress.new_value, __index = Progress.base_type})
return Progress

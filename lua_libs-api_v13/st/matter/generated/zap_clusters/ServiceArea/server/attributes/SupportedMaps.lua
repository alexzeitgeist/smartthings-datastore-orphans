...
--- @field public NAME string "SupportedMaps" the name of this attribute
...
local SupportedMaps = {
  ...
  element_type = require "st.matter.generated.zap_clusters.ServiceArea.types.MapStruct",
}

--- Add additional functionality to the base type object
---
--- @param base_type_obj st.matter.data_types.Array the base data type object to add functionality to
function SupportedMaps:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, SupportedMaps.element_type)
  end
end
...
function SupportedMaps:new_value(...)
...
function SupportedMaps:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function SupportedMaps:subscribe(device, endpoint_id)
...
function SupportedMaps:set_parent_cluster(cluster)
...
function SupportedMaps:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function SupportedMaps:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(SupportedMaps, {__call = SupportedMaps.new_value, __index = SupportedMaps.base_type})

...
--- @class st.matter.clusters.TargetNavigator.CurrentTarget
--- @alias CurrentTarget
---
--- @field public ID number 0x0001 the ID of this attribute
--- @field public NAME string "CurrentTarget" the name of this attribute
...
function CurrentTarget:read(device, endpoint_id)
...
--- Reporting policy: CurrentTarget => true => mandatory
...
--- @param endpoint_id number|nil
--- @return any
function CurrentTarget:subscribe(device, endpoint_id)
...
function CurrentTarget:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

--- Builds an CurrentTarget test attribute reponse for the driver integration testing framework
---
...
function CurrentTarget:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function CurrentTarget:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(CurrentTarget, {__call = CurrentTarget.new_value, __index = CurrentTarget.base_type})

...
--- @field public NAME string "AcceptHeader" the name of this attribute
...
function AcceptHeader:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, AcceptHeader.element_type)
  end
end
...
function AcceptHeader:new_value(...)
...
function AcceptHeader:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function AcceptHeader:subscribe(device, endpoint_id)
...
function AcceptHeader:set_parent_cluster(cluster)
...
function AcceptHeader:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function AcceptHeader:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(AcceptHeader, {__call = AcceptHeader.new_value, __index = AcceptHeader.base_type})
return AcceptHeader

...
WiredFaultChange.field_defs = {
  {
    ...
    element_type = require "st.matter.generated.zap_clusters.PowerSource.types.WiredFaultEnum",
  },
  {
    name = "previous",
    field_id = 1,
...
function WiredFaultChange:augment_type(base_type_obj)
  local elems = {}
...
function WiredFaultChange:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function WiredFaultChange:subscribe(device, endpoint_id)
...
function WiredFaultChange:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "WiredFaultChangeEventData", ID = 0x15}))
...
function WiredFaultChange:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function WiredFaultChange:set_parent_cluster(cluster)

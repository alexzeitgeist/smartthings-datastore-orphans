...
--- @class st.matter.clusters.PowerSource.EndpointList
--- @alias EndpointList
---
--- @field public ID number 0x001F the ID of this attribute
--- @field public NAME string "EndpointList" the name of this attribute
...
function EndpointList:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, EndpointList.element_type)
  end
end
...
function EndpointList:new_value(...)
...
function EndpointList:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function EndpointList:subscribe(device, endpoint_id)
...
function EndpointList:set_parent_cluster(cluster)
...
function EndpointList:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function EndpointList:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(EndpointList, {__call = EndpointList.new_value, __index = EndpointList.base_type})
return EndpointList

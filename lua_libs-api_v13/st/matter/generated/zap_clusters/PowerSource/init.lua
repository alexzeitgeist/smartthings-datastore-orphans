...
local PowerSourceEvents = require "st.matter.generated.zap_clusters.PowerSource.server.events"
...
PowerSource.server.events = PowerSourceEvents:set_parent_cluster(PowerSource)
...
function PowerSource:get_event_by_id(event_id)
  local event_id_map = {
    [0x0000] = "WiredFaultChange",
    [0x0001] = "BatFaultChange",
    [0x0002] = "BatChargeFaultChange",
  }
...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.PowerSource.server.attributes")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      PowerSource.attribute_direction_map[alias] = "server"
    end
  end
end
...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.PowerSource.server.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      PowerSource.command_direction_map[alias] = "server"
    end
  end
end

do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.PowerSource.client.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      PowerSource.command_direction_map[alias] = "client"
    end
  end
end

PowerSource.FeatureMap = PowerSource.types.Feature

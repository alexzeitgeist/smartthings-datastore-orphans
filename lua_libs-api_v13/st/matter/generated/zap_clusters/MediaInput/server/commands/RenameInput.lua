...
-----------------------------------------------------------
-- MediaInput command RenameInput
-----------------------------------------------------------

--- @class st.matter.clusters.MediaInput.RenameInput
--- @alias RenameInput
---
--- @field public ID number 0x0003 the ID of this command
--- @field public NAME string "RenameInput" the name of this command
...
RenameInput.ID = 0x0003
RenameInput.field_defs = {
  {
    name = "index",
    field_id = 0,
    is_nullable = false,
...
--- Builds an RenameInput test command reponse for the driver integration testing framework
---
...
function RenameInput:build_test_command_response(device, endpoint_id, status)
...
--- Initialize the RenameInput command
---
--- @param self RenameInput the template class for this command
--- @param device st.matter.Device the device to build this message to
...
function RenameInput:init(device, endpoint_id, index, name)
  local out = {}
  ...
  setmetatable(out, {
    __index = RenameInput,
    __tostring = RenameInput.pretty_print
  })
...
function RenameInput:set_parent_cluster(cluster)
...
function RenameInput:augment_type(base_type_obj)
  local elems = {}
...
function RenameInput:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(RenameInput, {__call = RenameInput.init})

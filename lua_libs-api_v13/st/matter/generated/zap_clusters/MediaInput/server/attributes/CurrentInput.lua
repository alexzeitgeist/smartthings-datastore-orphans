...
--- @class st.matter.clusters.MediaInput.CurrentInput
--- @alias CurrentInput
---
--- @field public ID number 0x0001 the ID of this attribute
--- @field public NAME string "CurrentInput" the name of this attribute
...
function CurrentInput:read(device, endpoint_id)
...
--- Reporting policy: CurrentInput => true => mandatory
...
--- @param endpoint_id number|nil
--- @return any
function CurrentInput:subscribe(device, endpoint_id)
...
function CurrentInput:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

--- Builds an CurrentInput test attribute reponse for the driver integration testing framework
---
...
function CurrentInput:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function CurrentInput:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(CurrentInput, {__call = CurrentInput.new_value, __index = CurrentInput.base_type})
return CurrentInput

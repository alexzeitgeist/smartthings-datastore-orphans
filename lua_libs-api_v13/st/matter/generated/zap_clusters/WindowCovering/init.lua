...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.WindowCovering.server.attributes")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      WindowCovering.attribute_direction_map[alias] = "server"
    end
  end
end
...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.WindowCovering.server.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      WindowCovering.command_direction_map[alias] = "server"
    end
  end
end

do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.WindowCovering.client.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      WindowCovering.command_direction_map[alias] = "client"
    end
  end
end

WindowCovering.FeatureMap = WindowCovering.types.Feature

function WindowCovering.are_features_supported(feature, feature_map)
  if (WindowCovering.FeatureMap.bits_are_valid(feature)) then

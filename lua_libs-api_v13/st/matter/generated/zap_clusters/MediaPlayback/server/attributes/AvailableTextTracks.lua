...
--- @class st.matter.clusters.MediaPlayback.AvailableTextTracks
--- @alias AvailableTextTracks
---
--- @field public ID number 0x000A the ID of this attribute
--- @field public NAME string "AvailableTextTracks" the name of this attribute
--- @field public data_type st.matter.data_types.Array the data type of this attribute

local AvailableTextTracks = {
  ID = 0x000A,
...
function AvailableTextTracks:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, AvailableTextTracks.element_type)
  end
end
...
function AvailableTextTracks:new_value(...)
...
function AvailableTextTracks:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function AvailableTextTracks:subscribe(device, endpoint_id)
...
function AvailableTextTracks:set_parent_cluster(cluster)
...
function AvailableTextTracks:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function AvailableTextTracks:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(AvailableTextTracks, {__call = AvailableTextTracks.new_value, __index = AvailableTextTracks.base_type})

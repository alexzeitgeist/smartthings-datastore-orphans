...
local OccupancySensingEvents = require "st.matter.generated.zap_clusters.OccupancySensing.server.events"
...
OccupancySensing.server.events = OccupancySensingEvents:set_parent_cluster(OccupancySensing)
...
function OccupancySensing:get_event_by_id(event_id)
  local event_id_map = {
    [0x0000] = "OccupancyChanged",
  }
...
do
  ...
  if has_aliases then
    for alias, _ in pairs(aliases) do
      OccupancySensing.attribute_direction_map[alias] = "server"
    end
  end
end
...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.OccupancySensing.server.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      OccupancySensing.command_direction_map[alias] = "server"
    end
  end
end

do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.OccupancySensing.client.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      OccupancySensing.command_direction_map[alias] = "client"
    end
  end
end

OccupancySensing.FeatureMap = OccupancySensing.types.Feature

function OccupancySensing.are_features_supported(feature, feature_map)
  if (OccupancySensing.FeatureMap.bits_are_valid(feature)) then

...
--- @alias PresetTypes
---
--- @field public ID number 0x0048 the ID of this attribute
--- @field public NAME string "PresetTypes" the name of this attribute
...
local PresetTypes = {
  ...
  element_type = require "st.matter.generated.zap_clusters.Thermostat.types.PresetTypeStruct",
}

--- Add additional functionality to the base type object
---
--- @param base_type_obj st.matter.data_types.Array the base data type object to add functionality to
function PresetTypes:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, PresetTypes.element_type)
  end
end
...
function PresetTypes:new_value(...)
...
function PresetTypes:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function PresetTypes:subscribe(device, endpoint_id)
...
function PresetTypes:set_parent_cluster(cluster)
...
function PresetTypes:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function PresetTypes:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(PresetTypes, {__call = PresetTypes.new_value, __index = PresetTypes.base_type})
return PresetTypes

...
--- @class st.matter.clusters.AudioOutput.CurrentOutput
--- @alias CurrentOutput
---
--- @field public ID number 0x0001 the ID of this attribute
--- @field public NAME string "CurrentOutput" the name of this attribute
...
function CurrentOutput:read(device, endpoint_id)
...
--- Reporting policy: CurrentOutput => true => mandatory
...
--- @param endpoint_id number|nil
--- @return any
function CurrentOutput:subscribe(device, endpoint_id)
...
function CurrentOutput:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

--- Builds an CurrentOutput test attribute reponse for the driver integration testing framework
---
...
function CurrentOutput:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function CurrentOutput:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(CurrentOutput, {__call = CurrentOutput.new_value, __index = CurrentOutput.base_type})

...
--- @class st.matter.clusters.ApplicationLauncher.CatalogList
--- @alias CatalogList
---
--- @field public ID number 0x0000 the ID of this attribute
--- @field public NAME string "CatalogList" the name of this attribute
...
function CatalogList:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, CatalogList.element_type)
  end
end
...
function CatalogList:new_value(...)
...
function CatalogList:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function CatalogList:subscribe(device, endpoint_id)
...
function CatalogList:set_parent_cluster(cluster)
...
function CatalogList:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function CatalogList:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(CatalogList, {__call = CatalogList.new_value, __index = CatalogList.base_type})
return CatalogList

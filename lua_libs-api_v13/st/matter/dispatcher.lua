...
function MatterMessageDispatcher.init(cls, name, dispatcher_filter, default_handlers)
  ...
  new_dispatcher:standardize_default_handlers()
  return new_dispatcher
end

--- Standardize the handlers in a dispatcher by wrapping each handler in
--- the corresponding AttributeReportHandler, EventReportHandler, or
--- CommandResponseHandler.
---
function MatterMessageDispatcher:standardize_default_handlers()
  utils.merge(self.default_handlers, {attr = {}, cmd_response = {}, event = {}})

  local new_attr = {}
  for cluster_id, attrs in pairs(self.default_handlers.attr) do
    new_attr[cluster_id] = {}
    for attr_id, handlers in pairs(attrs) do
      new_attr[cluster_id][attr_id] = AttributeReportHandler(cluster_id, attr_id, handlers)
    end
  end
  self.default_handlers.attr = new_attr

  local new_event = {}
  for cluster_id, evts in pairs(self.default_handlers.event) do
  ...
  self.default_handlers.event = new_event

  local new_cmd_resp = {}
  for cluster_id, cmds in pairs(self.default_handlers.cmd_response) do
  ...
  self.default_handlers.cmd_response = new_cmd_resp
end

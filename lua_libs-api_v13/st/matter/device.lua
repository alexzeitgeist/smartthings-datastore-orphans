...
local constants = require "st.matter.constants"
...
--- By default, only server type clusters will be returned unless otherwise specified with the
--- "cluster_type" optional parameter.
---
--- @param cluster_id number|nil the cluster ID to check for (if nil, all endpoint ids are returned)
--- @param opts table|nil valid options include feature_bitmap to check cluster feature support, and cluster_type (using st.matter.constants.CLUSTER_TYPE)
  ...
  for _, ep in ipairs(self.endpoints) do
    for _, clus in ipairs(ep.clusters) do
      if ((clus.cluster_id == cluster_id)
            ...
            and ((opts.cluster_type == nil and clus.cluster_type == constants.CLUSTER_TYPE.SERVER or clus.cluster_type == "BOTH")

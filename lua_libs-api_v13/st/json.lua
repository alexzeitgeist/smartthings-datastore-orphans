...
--- @class JsonModule
---
--- The json module is what is required into a driver. It loads a native json module when running on target, or dkjson,
--- a pure lua implementation, when the native json impl is not available (i.e running
--- driver unit tests). dkjson can be used on target by requiring it instead of this module,
--- but the native json module has much better performance than dkjson in terms of memory usage.
--- The api's are mostly interchangable, however, not all options in dkjson are available
--- in the native json module. See https://github.com/LuaDist/dkjson for dkjson documentation,
--- and the Edge Driver reference for the native json documentation.

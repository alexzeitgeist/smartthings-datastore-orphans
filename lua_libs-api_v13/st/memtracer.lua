...
memtracer.instrumented_function = {
    registered_fns = {}
}
...
function memtracer.instrumented_function.calculate_std_dev(samples, mean)
    local sum = 0
    local count = 0
...
function memtracer.instrumented_function.calculate_median(samples)
    if not samples then
        return 0
    end
...
function memtracer.instrumented_function.generate_report(metrics)
    local total = 0
    if #(metrics.samples or {}) == 0 then
        return nil, "empty report"
    end
    ...
    local median = memtracer.instrumented_function.calculate_median(metrics.samples)
    local mean = total / #metrics.samples
    local std_dev = memtracer.instrumented_function.calculate_std_dev(metrics.samples, mean)
    return {
        name = metrics.name,
        min = min,
        max = max,
...
function memtracer.instrument_and_register(other_func, name)
    if not memtracer.instrumented_function.registered_fns[name] then
        local fn, metrics = memtracer.instrument_function(other_func, name)
        memtracer.instrumented_function.registered_fns[name] = {fn, metrics}
    end
    return table.unpack(memtracer.instrumented_function.registered_fns[name])
end
...
function memtracer.instrument_function(other_func, name)
    ...
    local metrics = {
        ...
        samples = {},
        last_sample_index = 1,
        generate_report = memtracer.instrumented_function.generate_report,
    }

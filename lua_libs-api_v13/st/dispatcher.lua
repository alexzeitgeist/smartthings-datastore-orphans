...
--- @field public lazy_loaded boolean a flag to check if handlers have been loaded in the case of a lazy subdriver load
...
function MessageDispatcher.init(cls, name, dispatcher_filter, default_handlers, dispatcher_class_name)
  ...
  new_dispatcher.lazy_loaded = false
...
--- @return table list of this dispatcher's child dispatchers that can handle this message
function MessageDispatcher:can_handle(driver, device, ...)
  local status, val, subdriver = pcall(self.dispatcher_filter, {dispatcher_class = self.dispatcher_class_name}, driver, device, ...)
  -- subdrivers that lazy load will return the subdriver here
  if not status then
    log.warn(string.format("Encountered error calling can_handle for dispatcher defaulting to false: %s", val))
  end
  if status and val and subdriver ~= nil and self.lazy_loaded == false then
    -- if can_handle is true, then load subdriver handlers
    if self.dispatcher_class_name == "CapabilityCommandDispatcher" and subdriver.capability_handlers ~= nil then
      self.default_handlers = utils.deep_copy(subdriver.capability_handlers)
    elseif self.dispatcher_class_name == "DeviceLifecycleDispatcher" and subdriver.lifecycle_handlers ~= nil then
      self.default_handlers = utils.deep_copy(subdriver.lifecycle_handlers)
    elseif self.dispatcher_class_name == "ZwaveDispatcher" and subdriver.zwave_handlers ~= nil then
      local ZwaveDriver = require "st.zwave.driver"
      -- extend wakeup handler for sleepy zwave devices
      ZwaveDriver.extend_wakeup_handler(subdriver)
      self.default_handlers = utils.deep_copy(subdriver.zwave_handlers)
    elseif self.dispatcher_class_name == "ZigbeeMessageDispatcher" and subdriver.zigbee_handlers ~= nil then
      self.default_handlers = utils.deep_copy(subdriver.zigbee_handlers)
      self:standardize_default_handlers()
    elseif self.dispatcher_class_name == "MatterMessageDispatcher" and subdriver.matter_handlers ~= nil then
      self.default_handlers = utils.deep_copy(subdriver.matter_handlers)
      self:standardize_default_handlers()
    end
    device.log.info_with({hub_logs = true}, "Lazy loaded driver into dispatcher: " .. self.name)
    self.lazy_loaded = true
  end

  local child_dispatchers = {}
  local handler_found = false
  if status and val then
    child_dispatchers = self:find_child_dispatchers(driver, device, ...)
    handler_found =  (#child_dispatchers > 0) or (#self:find_default_handlers(driver, device, ...) > 0)
  end
  return handler_found, child_dispatchers
end
...
local function inner_dispatch(self, driver, device, child_dispatchers, ...)
  if child_dispatchers ~= nil and #child_dispatchers > 0 then
    for _, dispatcher in ipairs(child_dispatchers) do
...
function MessageDispatcher:dispatch(driver, device, ...)
  local handler_found, child_dispatchers = self:can_handle(driver, device, ...)
  if handler_found then
    local status, value = pcall(inner_dispatch, self, driver, device, child_dispatchers, ...)
    if not status then
      if type(self.error_handler) == "function" then
        self.error_handler(driver, device, ..., value)
      else
        ...
        for i, arg in ipairs(args) do
          ...
          if type(arg) == "table" then
            arg_str = string.format("    arg%d: %s\n", i, utils.stringify_table(arg))
          else
            arg_str = string.format("    arg%d: %s\n", i, tostring(arg))
          end
        ...
        if type(value) ~= "nil" then
          error_msg = error_msg .. utils.stringify_table(value)
        end
        error(error_msg)
      end
    end
  elseif type(self.fallback_handler) == "function" then

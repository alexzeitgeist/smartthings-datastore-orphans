...
function utils.get_print_safe_string(str, force_bin)
  if str:match("^[%g%s]+$") ~= nil and not force_bin then
...
---@param str string
---@return string ascii hex string
function utils.bytes_to_hex_string(str)
...
---
---@param value number the signed value
---@param size number the size of the value (in bytes)
---@return number the converted value
function utils.signed_to_unsigned(value, size)
  if value >= 0 then
    return value
  else
    return value + 2^(size * 8)
  end
end

--- Convert an unsigned integer into a 2s complement signed integer
---
---@param value number the unsigned integer value
---@param size number the size of the value (in bytes)
---@return number the converted value
function utils.unsigned_to_signed(value, size)
  local msb = 2^(size * 8 - 1)
  if value >= msb then
    return value - 2^(size * 8)
  else
    return value

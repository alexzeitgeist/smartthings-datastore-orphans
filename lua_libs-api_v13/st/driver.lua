  ...
  if driver ~= nil and device ~= nil then
    ...
    then
      device.log.debug_with({ hub_logs = true },
        "added callback did not fail, executing init handler"
      )
...
---
--- This function handles the "secretData" security message that is returned asynchronously as a response to a request
--- from a driver. The secret data security message contains sensitive information (like keys or auth material)
--- obtained by the hub on behalf of the driver.
---
--- @param self Driver This driver
--- @param secret_data Table Secret data returned as the result from the get_secret request
function Driver:secret_data_handler(secret_data)
  if secret_data.secret_kind == nil then
    error("Could not determine kind of secret")
  end
  if not self.security then
    self.security = require("st.security")
  end
  local status, result = pcall(self.security.get_device_for_event, self, secret_data)
  if status then
    self.secret_data_dispatcher:dispatch(self, result, secret_data)
  else
    log.error_with({ hub_logs = true }, string.format("Failed to handle security event: %s", result))
  end
end

--- Default handler for security event messages
---
--- Security event messages are currently limited to secret_data requests (see `secret_data_handler`) which are
--- keys or auth material obtained by the hub on behalf of the driver.
---
--- @param self Driver the driver to handle the security events
--- @param security_channel message_channel the security message channel
function Driver:security_handler(security_channel)
  local event, data = security_channel:receive()
  if event == "secretData" then
    local status, secret_info = pcall(json.decode, data)
    if status then
      self._driver_thread:queue_event(self.secret_data_handler, self, secret_info)
    else
      error("Failed to decode secret data")
    end
  else
    log.error_with({hub_logs = true}, string.format("Unknown security event: %s %s", event, data))
  end
end
...
function Driver:try_create_device(device_metadata)
  ...
  required_metadata_fields_array = { "type", "label", "profile" }
...
---
--- @param driver Driver the driver
function Driver.populate_secret_data_dispatcher_from_sub_drivers(driver)
  for _, sub_driver in ipairs(driver.sub_drivers) do
    sub_driver.secret_data_handlers = sub_driver.secret_data_handlers or {}
    sub_driver.secret_data_dispatcher = SecretDataDispatcher(
        sub_driver.NAME,
        sub_driver.can_handle,
        sub_driver.secret_data_handlers
    )
    driver.secret_data_dispatcher:register_child_dispatcher(sub_driver.secret_data_dispatcher)
    Driver.populate_secret_data_dispatcher_from_sub_drivers(sub_driver)
  end
end
...
function Driver.init(cls, name, template)
  ...
  if out_driver.security_channel ~= nil then
    ...
    out_driver.populate_secret_data_dispatcher_from_sub_drivers(out_driver)
    ...
    out_driver:_register_channel_handler(
      ...
      template.security_handler or Driver.security_handler,
      "security"
    )
  end
  out_driver:_register_channel_handler(

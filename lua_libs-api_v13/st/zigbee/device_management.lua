...
function device_management.check_all_devices_for_health(driver)
  for uuid, device in pairs(driver.device_cache) do
    if type(device.check_monitored_attributes) == "function" then
      device:check_monitored_attributes()
    else
      -- Report a coroutine error with device info to figure out why this is happening.
      device.thread:call_with_delay(0, function()
        local dev_str = device.debug_pretty_print and device:debug_pretty_print() or device.network_type
        local err = string.format("check_monitored_attributes not available when expected for device: %s", dev_str)
        error(err)
      end, "no monitored attributes func error")
    end
  end
end

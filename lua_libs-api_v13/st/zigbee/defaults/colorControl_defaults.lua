...
function color_control_defaults.set_color(driver, device, command, register_native)
...
--- @param command CapabilityCommand The capability command table
--- @param register_native bool|nil Register future commands to be handled natively by the hub
function color_control_defaults.set_hue(driver, device, command, register_native)
  if register_native then
    device:register_native_capability_cmd_handler(command.capability, command.command)
  end
  switch_defaults.on(driver, device, command)
...
--- @param command CapabilityCommand The capability command table
--- @param register_native bool|nil Register future commands to be handled natively by the hub
function color_control_defaults.set_saturation(driver, device, command, register_native)
  if register_native then
    device:register_native_capability_cmd_handler(command.capability, command.command)
  end
  switch_defaults.on(driver, device, command)

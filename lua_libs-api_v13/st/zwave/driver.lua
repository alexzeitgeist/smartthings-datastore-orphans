...
---
--- @param driver st.zwave.Driver
function ZwaveDriver.extend_wakeup_handler(driver)
  ...
  if existing_wakeup_notification_handler ~= nil then --ensures subdrivers using parent handler does not override parent behavior
...
local function populate_zwave_dispatcher_from_sub_drivers(driver)
  for _, sub_driver in ipairs(driver.sub_drivers) do
    ...
    if Driver.should_lazy_load_sub_driver(sub_driver) then
      ...
      ZwaveDriver.extend_wakeup_handler(sub_driver)
      zwave_handlers = sub_driver.zwave_handlers
    end
...
local function validate_cmd_callbacks(driver)
  ...
  for cmd_class, v in pairs(driver.zwave_handlers) do
    if type(cmd_class) == "string" then
      assert((cmd_class == "fallback" or cmd_class == "error"), "The only reserved handlers are 'fallback' and 'error'")
      assert(type(v) == "function", "Fallback and error handlers must be functions")
    else
...
function ZwaveDriver.init(cls, name, driver_template)
  ...
  ZwaveDriver.extend_wakeup_handler(out_driver)
